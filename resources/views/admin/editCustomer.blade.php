<x-admin.layout>
@push('script')
    <script src="{{ asset('assets/admin/js/editCustomer.js') }}"></script>
@endpush
<section class="innerpages company-profile-main1 parallaxcont">
	<div class="container">
		...
		<form name="edit-customer" id="editCustomerForm" action="javascript:void(0);" data-url="{{ route('admin_customer_update') }}" data-url_redirect="{{ route('admin_customer_list') }}">
			@csrf
			<div class="mb-3">
			...
			<div class="mb-3">
				...
				<input type="email" name="email" class="form-control" id="email" placeholder="name@example.com" value="{{ old('email', $customer['email']) }}">
			...
			<div class="mb-3">
				...
				<textarea name="address" class="form-control" id="address">{{ old('address', $customer['address']) }}</textarea>
